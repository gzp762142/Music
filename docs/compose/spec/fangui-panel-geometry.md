...
T7 的假设（把本地窗口压到 `level -1` 就能只留托管那份）**被实机证伪**：

- `285503d` 录屏显示 **App 内看到的是「被转过」的那份，退出 App 后才是「正向」的那份** ——
  与「本地那份被盖住」的预期相反。

结构性原因（代码层面已确认，与设备无关）：

- 两份渲染来自**同一份 layer**，我们对窗口/内容施加的任何变换对两份影响相同；
- SpringBoard 还要再叠加一次屏幕旋转（横屏 +90°）。
- 因此两份朝向**必然相差 90°**：`T` 让托管那份正，本地那份就歪，反之亦然。
  ⇒ **只改变换永远只能对一份，必须让其中一份不参与合成。**

T8 的做法：本地层级做成**可在设备上循环切换**的运行期档位
（`sys / statusBar+1 / normal / -1 / -100`），双指长按面板循环。
不写死猜测值 —— 哪一档能让本地那份不被自己场景合成，只能在真机上试出来。
档位持久化到 `UserDefaults`，诊断行的 `lv` 字段显示当前档位名。
...
- [x] T7: 默认居中 + 注册层级与本地层级解耦 — acceptance: 面板正居中；SBS 注册仍为系统级 (covers: S2; depends: T5)
- [ ] T8: 本地层级运行期档位化（双指长按循环） — acceptance: 找到某一档时屏幕上只剩一份面板，且退出 App 后仍可见 (covers: S1.4, S2; depends: T7)
- [ ] T4: 推 CI 出包并在 iPad 上确认 — acceptance: CI 绿；装包后 4 项现象全部消失 (covers: S1; depends: T1, T2, T3, T5, T6, T7, T8)
